...
def admin_backend() -> None:
    ...
    with t3:
        ...
        def kpi_builder(domain_key: str):
            ...
            # Display KPIs (predefined) + enable/disable custom KPIs
            st.markdown("#### Display KPIs")
            defaults = load_kpis(cid3, domain_key) if cid3 else DEFAULT_KPIS[domain_key]
            # split defaults into predefined strings and custom dicts
            base_predef = [k for k in defaults if isinstance(k, str)]
            custom_existing = [k for k in defaults if isinstance(k, dict)]

            chosen_predef = st.multiselect(
                "Choose Predefined KPIs",
                options=DEFAULT_KPIS[domain_key],
                default=base_predef,
                key=f"kpi_predef_{domain_key}"
            )

            custom_names = [d.get("name", "Unnamed KPI") for d in custom_existing]
            chosen_custom_names = st.multiselect(
                "Enable Custom KPIs",
                options=custom_names,
                default=custom_names,
                key=f"kpi_custom_names_{domain_key}"
            )
            ...
            if st.button(f"➕ Add Custom KPI", key=f"add_custom_{domain_key}"):
                if kpi_name and kpi_col:
                    ...
                    # Auto-enable newly added KPI in-session
                    en_key = f"kpi_custom_names_{domain_key}"
                    st.session_state[en_key] = list(dict.fromkeys((st.session_state.get(en_key) or []) + [kpi_name]))
                    st.success("Custom KPI added and enabled (remember to Save KPIs).")
                else:
            ...
            if st.button("Add Column", key=f"add_col_{domain_key}"):
                try:
                    ...
                    en_key = f"kpi_custom_names_{domain_key}"
                    st.session_state[en_key] = list(dict.fromkeys((st.session_state.get(en_key) or []) + [new_col]))
                    st.success(f"Column '{new_col}' added, saved, and enabled as KPI (sum).")
                except Exception as e:
            ...
            with m2:
                m_expr = st.text_input("Measure formula (e.g., sum(total_spend)/sum(qty))", key=f"meas_expr_{domain_key}")
            with m3:
                st.markdown("&nbsp;")
                if st.button("Add Measure", key=f"add_meas_{domain_key}"):
                    custom_existing.append({"name": m_name, "expr": m_expr, "type": "measure"})
                    en_key = f"kpi_custom_names_{domain_key}"
                    st.session_state[en_key] = list(dict.fromkeys((st.session_state.get(en_key) or []) + [m_name]))
                    st.success("Measure added and enabled (remember to Save KPIs).")

            if st.button(f"Save {domain_key.title()} KPIs", key=f"save_{domain_key}_kpis"):
                if cid3:
                    # Persist predefined selections + the enabled custom KPI dicts (by name)
                    enabled_customs = [d for d in custom_existing if d.get("name") in (st.session_state.get(f"kpi_custom_names_{domain_key}") or [])]
                    final_kpis = list(chosen_predef) + enabled_customs
